...
if os.path.exists(data_path):
    ...
    n_events = (
        int(df["total_events"].sum()) if "total_events" in df.columns else "—"
    )
    avg_listen = (
...
# ═══════════════════════════════════════════════════════════════════════════
st.markdown(
    '<div class="section-title">🔭 What We Investigate</div>',
...
# ═══════════════════════════════════════════════════════════════════════════
st.markdown(
    '<div class="section-title">📂 The Three Analysis Areas</div>',
    unsafe_allow_html=True,
)
...
for col, icon, title, color, border, bg, desc, qs, page in [
    (
        ...
        [
            "Listeners vs. Tour Scale",
            "Streaming Concentration vs. Tour Intensity",
    ...
    (
        ...
        [
            "Revisit vs. New Cities",
            "Capital vs. Non-Capital Cities",
            "Streaming Countries vs. Tour Countries",
        ],
    ...
    with col:
        st.markdown(
            f"""<div style="{card_style.format(border=border, bg=bg)}">
            <div style="font-size:2.2rem;margin-bottom:14px;">{icon}</div>
            <div style="color:{color} !important;font-size:3rem;font-weight:700;
...
for col, icon, title, text in [
    (
        ...
        "📥",
        "Data Collection",
        "Last.fm API for popularity metrics, Ticketmaster for concert data, RestCountries for capital city information, Spotify Charts for chart classification.",
    ),
    (
        me2,
        "🔗",
        "Data Join",
        "Linked via normalised artist name. 499 artists with complete data available in both sources.",
    ),
    (
        me3,
        "📊",
        "Analysis",
        "Trend lines, group comparisons, and distribution analysis across popularity tiers and geographic dimensions.",
    ),
    (
        me4,
        "🖥️",
        "Visualisation",
        "Interactive Plotly charts embedded in Streamlit. All charts are filterable and zoomable.",
    ),
]:
    with col:
        st.markdown(
            f"""<div style="background:#161c2d;border:1px solid #232840;border-radius:10px;
