...
SUMMARY_PATH = os.path.join(PLOT_DIR, "rq1_summary.png")
...
# -----------------------------
# GRAPH 4: SUMMARY PANEL
# -----------------------------
import matplotlib.pyplot as plt

quartile_values = quartile_summary["mean"].tolist()

summary_text = f"""
RQ1: Streaming vs Tour Scale

Pearson Correlation (listeners vs events):
r = {r:.3f}
...
Average events per listener group:
G1: {quartile_values[0]}
G2: {quartile_values[1]}
G3: {quartile_values[2]}
G4: {quartile_values[3]}

Interpretation:
Listener count is not a strong predictor
for the number of scheduled tour events.
"""
...
plt.text(
    0.05,
    0.95,
    "RQ1 Summary – Streaming & Ticket Power",
    fontsize=18,
    fontweight="bold",
